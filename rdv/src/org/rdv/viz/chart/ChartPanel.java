 ...
 *                   Onno vd Akker;
 *                   Sergei Ivanov;
 *                   Jason P. Hanley (for Palta Software)
 *
 ...
 * 07-Jul-2008 : Added zoom-out via mouse, based on zoom-in history (JPH);
 *
 */
...
public class ChartPanel extends JPanel implements ChartChangeListener,
    ...
    /** Parent data panel (used for popup menu). */  
    private DataPanel parent_;
    
    private boolean popupMenuProperties_ = true;
    
    private boolean popupMenuSave_ = true;
     
    private boolean popupMenuPrint_=true;
    
    private boolean popupMenuZoom_=true;
    
    private boolean popupMenuTooltips_=true;
    
    /**
     * The history of domain and ranges ranges used for zooming out. There are
     * two ranges pushed onto the stack for every zoom-in. The range for the
     * range will always be at the top of the stack followed by the range for
     * the domain.
     */
    private Stack<Range> rangeHistory;
...
//    }
     ...
     * @param useBuffer  a flag controlling whether or not an off-screen buffer
     *                   is used.
     * @param popupBuilder a menu builder used to change the popup menu dynamically
     *                  before display.
     */
   public ChartPanel(JFreeChart chart, boolean useBuffer, 
          DataPanel parent) {
...
//    }
    ...
    public ChartPanel(JFreeChart chart,
        ...
        this.parent_=parent;
        popupMenuProperties_ = properties;
        popupMenuSave_ = save;
        popupMenuPrint_=print;
        popupMenuZoom_=zoom;
...
//                        Transparency.TRANSLUCENT);
    ...
    public void mousePressed(MouseEvent e) {
        if (this.zoomRectangle == null) {
            ...
            if (e.isPopupTrigger()) {
                // show popup
                parent_.getPopupMenuMouseListener().mousePressed(e);
            }
        }
    }
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (this.zoomRectangle != null) {
          ...
          parent_.getPopupMenuMouseListener().mouseReleased(e);
        }
    ...
    /**
     * Add popup menu items for the chart. Optionally include 
     * items for chart property editor, saving the chart, 
     * printing the chart, and zooming. Modify the zooming options 
     * depending on the type of chart being displayed by the panel.
     */
    public void buildPopupMenu(JPopupMenu result, MouseEvent e) {
  
        boolean separator = false;

        if (popupMenuProperties_) {
